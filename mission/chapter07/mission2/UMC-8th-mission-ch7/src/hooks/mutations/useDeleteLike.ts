...
function useDeleteLike() {
    return useMutation({
        ...
        // UI에 바로 변경을 보여주기 위해 Cache 업데이트를 직접 해줄 수 있음
        onMutate: async (lp) => {
            ...
            // 게시글 데이터를 복사해서 NewLpPost라는 새로운 객체를 만들것임
            // 복사하는 가장 큰 이유는 나중에 오류가 발생했을 때 이전 상태로 되돌리기 위해서
            const newLpPost = { ...previousLpPost }

            // 게시글에 저장된 좋아요 목록에서 현재 내가 눌렀던 좋아요의 위치를 찾아야.
        ...
        //onSettles는 API 요청이 끝난 후 (성공하든 실패하든 실행)
